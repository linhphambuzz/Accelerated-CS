...
static void treeFactory(GenericTree<int>& tree) {
  ...
  auto A=rootNodePtr->addChild(8);
  rootNodePtr->addChild(15);
  auto B=A->addChild(16); 
  A->addChild(23); 
  B->addChild(42);
  ...
  //       if (!showStem) {stemSymbol = " ";}

  //       bool isLastCol = false;
  //       if (stemIt + 1 == curMargin.end()) { isLastCol = true;}
  ...
  //         }
  //         //not last row, show | and go to a new line 
  //         else if (showStem){
  ...
  //         std::cout << stemSymbol << " "; 
  //       }
  //     // end of loop for margin stems
  //     }
  //     //end of loop for multi rows display 
  //   }
  //   if (curNode){
  ...
  //   //append childrens to the stack
  //   if (curNode->childrenPtrs.size()>0){
  //     // traverse right most child first, to left child : 
  //     //FILO for stacks, right most child gets printed last
  ...
  //       // margin of child is taken from traling margin of parents 
  //       auto nextMargin = trailingMargin;

  //       //margin for the child's node 
  //       nextMargin.push_back(true); //slot to print |_
  //       curMarginStack.push(nextMargin); 

  //       // trailing margin for the child's node 
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!q.empty()){
    ...
    if (node->childrenPtrs.size()>0){
      for (auto it = node->childrenPtrs.begin(); it != node->childrenPtrs.end(); it++){
        TreeNode* child=*it; 
        q.push(child); 
      }
    } 
