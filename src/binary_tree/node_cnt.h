...
int count(Node *n) {
  ...
  if (n->left==nullptr && n->right==nullptr){cnt=1;}
  else if (n->left!=nullptr && n->right==nullptr){cnt= 1+ count(n->left);}
  else if(n->left==nullptr && n->right!=nullptr){cnt= 1+count(n->right);}
  else {cnt=1+count(n->left)+count(n->right);}
