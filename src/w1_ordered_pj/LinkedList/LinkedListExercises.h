...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // with head 
    ...
    Node* curr=head_ ; // temp node to 1st pnt to head
    Node* prevv=head_;  //prev node 
    while (curr){
      std::cout<< "at " << curr->data <<std::endl;
      //find its position by comparing to curr's data 
      if (curr->data >= toInsert->data){
        // insert front 
        if (curr==head_){
          std::cout<< "insert at head with val" << toInsert->data << std::endl; 
          Node* oldHead=head_;
          toInsert->next=oldHead; 
          oldHead->prev=toInsert; 
          head_=toInsert;
          break;
          }
        // insert back 
        else if (curr==tail_){
          std::cout<< "insert at tail with val" << toInsert->data << std::endl; 
          toInsert->prev=prevv; 
          prevv->next=toInsert; 
          tail_->prev=toInsert; 
          toInsert->next=tail_;
        ...
        //middle 
          ...
          std:: cout<< "inserting at" << curr->data << std::endl;
          toInsert->prev=prevv; 
          prevv->next=toInsert;
          curr->prev=toInsert;  
          toInsert->next=curr; 
      ...
      //not the right pos. yet, move to next item 
      else{
        prevv=curr; 
        curr=curr->next;
      }
    }
    // traversed thru whole list, no pos. found. push towards end of list 
    if (prevv==tail_){
      std::cout<< "adding at tail"<<std::endl; 
      Node *oldTail=tail_;
      oldTail->next=toInsert; 
      toInsert->prev=oldTail;
      tail_=toInsert;
    }
  }
  //update size
  ...
  size_++;
  std::cout<<"updating size to "<< size_ <<std::endl; 
}
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // the function.
  LinkedList<T> merged;
  Node* l_curr=left.head_;
  Node* r_curr=right.head_; 
 // merged.size_=left.size_+right.size_; 

  if (left.size_==0 && right.size_!=0){
    for (int j=0;j<right.size_;j++){
      merged.pushBack(r_curr->data); 
      r_curr=r_curr->next;
    
    }
  }
  else if (right.size_==0 && left.size_!=0){
    for (int i=0;i<left.size_;i++){
      merged.pushBack(l_curr->data); 
      l_curr=l_curr->next; 
