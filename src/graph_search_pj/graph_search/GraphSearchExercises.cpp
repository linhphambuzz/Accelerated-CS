...
int GridGraph::countEdges() const {
  ...
  // TODO: Your code here!
  // =======================================================================
  std::unordered_set<IntPairPair> edges; 
  for (const auto& node : adjacencyMap){
  
    for (auto it=node.second.begin();it!=node.second.end();it++ ){
      auto nb=*it; 
      IntPairPair new_edge= std::make_pair(node.first,nb);
      IntPairPair r_new_edge=std::make_pair(nb,node.first) ; 
      if (!edges.count(r_new_edge)){ edges.insert(new_edge); }
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // TODO: Your code here!
  // =======================================================================
  for (auto& nb:originalNeighbors){
    adjacencyMap.at(nb).erase(p1);
  }
